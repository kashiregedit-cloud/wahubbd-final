...
export class HttpsExpress {
  ...
  readSync() {
    ...
    this.logger.info({ keyPath: this.keyPath }, 'HTTPS Key Path');
    const key = fs.readFileSync(this.keyPath);

    this.logger.info({ certPath: this.certPath }, 'HTTPS Cert Path');
    const cert = fs.readFileSync(this.certPath);

    this.logger.info({ caPath: this.caPath }, 'HTTPS CA Path');
