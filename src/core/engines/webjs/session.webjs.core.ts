...
export class WhatsappSessionWebJSCore extends WhatsappSession {
  ...
  protected addProxyConfig(clientOptions: any) {
  ...
  private async end() {
    ...
    try {
      // @ts-ignore
      const strategy = this.whatsapp?.authStrategy as any;
  ...
  protected listenConnectionEvents() {
    ...
    this.whatsapp.on(Events.STATE_CHANGED, (state: any) => {
  ...
  async sendVoice(request: MessageVoiceRequest) {
    ...
    const content =
      request.convert && this.mediaConverter
        ? await this.mediaConverter.voice(buffer)
        : buffer;
    const media = new MessageMedia(
      mimetype,
  ...
  async sendSeen(request: SendSeenRequest) {
    const chat = (await this.whatsapp.getChatById(
      this.ensureSuffix(request.chatId),
    )) as any;
    await chat.sendSeen();
  }
   ...
   */
  private recreateMessage(msgId: string): any {
  ...
  public async getInfoAdminsOnly(id): Promise<SettingsSecurityChangeInfo> {
    const groupChat = (await this.whatsapp.getChatById(id)) as any;
    return {
  ...
  public async getGroupParticipants(id: string): Promise<GroupParticipant[]> {
    const group = (await this.whatsapp.getChatById(id)) as any;
  ...
  protected ChatToChannel(chat: any): Channel {
    // @ts-ignore
  ...
  subscribeEngineEvents2() {
    ...
    for (const key in Events) {
      const event = Events[key];
      const event$ = fromEvent(this.whatsapp as any, event);
      events.push(
        event$.pipe(
          map((data) => {
            return {
    ...
    // Messages
    //
    const messageReceived$ = fromEvent(this.whatsapp as any, Events.MESSAGE_RECEIVED);
    const messagesFromOthers$ = messageReceived$.pipe(
      filter((msg: any) => this.jids.include(msg?.id?.remote)),
      mergeMap((msg: any) => this.processIncomingMessage(msg, true)),
      share(),
    );
    this.events2.get(WAHAEvents.MESSAGE).switch(messagesFromOthers$);

    const messageCreate$ = fromEvent(this.whatsapp as any, Events.MESSAGE_CREATE);
    ...
    const messageReaction$ = fromEvent(this.whatsapp as any, 'message_reaction');
    const messagesReaction$ = messageReaction$.pipe(
      filter((reaction: any) => this.jids.include(reaction?.id?.remote)),
    ...
    const pollVote$ = fromEvent(this.whatsapp as any, Events.VOTE_UPDATE);
    ...
    const tagReceiptNode$ = fromEvent(this.whatsapp as any, Events.TAG_RECEIPT);
    ...
    // Others
    //
    const stateChanged$ = fromEvent(this.whatsapp as any, Events.STATE_CHANGED);
    this.events2.get(WAHAEvents.STATE_CHANGE).switch(stateChanged$);

    //
    // Presence
    //
    const tagPresenceNode$ = fromEvent(this.whatsapp as any, Events.TAG_PRESENCE);
    const presences$ = tagPresenceNode$.pipe(
      map(TagPresenceToPresence),
      filter(Boolean),
      filter((presence: any) => this.jids.include(presence.id)),
    );
    const tagChatstateNode$ = fromEvent(this.whatsapp as any, 'tag:chatstate');
    ...
    // Groups
    //
    const groupJoin$ = fromEvent(this.whatsapp as any, Events.GROUP_JOIN);
    ...
    const groupLeave$ = fromEvent(this.whatsapp as any, Events.GROUP_LEAVE);
    ...
    const groupUpdate$ = fromEvent(this.whatsapp as any, Events.GROUP_UPDATE);
    ...
    // Calls
    //
    const call$ = fromEvent(this.whatsapp as any, 'call');
    const calls$ = call$.pipe(
      filter((call: any) => this.jids.include((call as any)?.from)),
      map((call: any) => {
        return {
          id: call.id,
          from: call.from,
  ...
  private processMessageReaction(reaction: any): WAMessageReaction {
    if (this.lastQRDate) {
  ...
  private toPollVotePayload(vote: any): PollVotePayload | null {
  ...
  protected async downloadMedia(message: any) {
...
export class WEBJSEngineMediaProcessor
  implements IMediaEngineProcessor<any>
{
  ...
  async getMediaBuffer(message: any): Promise<Buffer | null> {
    return message.downloadMedia().then((media: any) => {
      if (!media) {
        return null;
      }
