...
export abstract class WhatsappSession {
  ...
  async maintainPresenceOnline(): Promise<void> {
    ...
    if (this._presence !== WAHAPresenceStatus.ONLINE) {
      try {
        ...
        this.logger.debug(
          { name: this.name },
          'Set presence to ONLINE due to activity',
        );
      } catch (error) {
        this.logger.debug(
          { name: this.name, err: error },
          'Failed to set presence ONLINE',
        );
        return;
      }
    }
    ...
    this.presenceOfflineTimeout = setTimeout(async () => {
      try {
        ...
        this.logger.debug(
          {
            name: this.name,
            duration: this.presenceAutoOnlineConfig.duration,
          },
        ...
        this.logger.debug(
          { name: this.name, err: error },
          'Failed to set presence OFFLINE',
        );
      }
      this.cleanupPresenceTimeout();
  ...
  protected async readChatMessagesWSImpl(
    ...
    const messagesDevice = await this.getChatMessages(chatId, query, filter);
    const filterServer: GetChatMessagesFilter = {
      ...filter,
      'filter.ack': WAMessageAck.SERVER,
    };
    const messagesServer = await this.getChatMessages(
      chatId,
      query,
      filterServer,
    );
    const idsSet = new Set<string>();
    for (const m of messagesDevice) idsSet.add(m.id);
    for (const m of messagesServer) idsSet.add(m.id);
    const ids = Array.from(idsSet);
    this.logger.debug(`Found ${ids.length} messages to read`);
    if (ids.length === 0) {
      return { ids: [] };
    }
  ...
  protected deserializeId(messageId: string): WebjsMessageId {
